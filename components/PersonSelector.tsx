  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        className={`w-full flex items-center gap-2 bg-white/60 border rounded-lg px-2 py-1.5 text-xs shadow-sm transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-amber-500/20 group
      ...
      >
        <div className="relative shrink-0">
          <div
            className={`size-6 rounded-full flex items-center justify-center text-[10px] font-bold overflow-hidden ring-1 ring-white shadow-xs
            ${
              currentPerson
          ...
          {currentPerson && (
            <div
              className={`absolute -bottom-0.5 -right-0.5 size-3 rounded-full ring-1 ring-white shadow-xs flex items-center justify-center ${getGenderStyle(currentPerson.gender)}`}
            >
              {currentPerson.gender === "male" ? (
                <MaleIcon className="size-2" />
              ) : currentPerson.gender === "female" ? (
                <FemaleIcon className="size-2" />
              ) : null}
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0 text-left">
          {label && (
            <p className="text-[9px] font-bold text-stone-400 uppercase tracking-widest leading-none mb-0.5">
              {label}
            </p>
          )}
          <p className="truncate text-stone-800 font-bold select-none leading-tight">
            {currentPerson
        ...
        >
          <ChevronDown
            className={`size-3 shrink-0 transition-colors ${isOpen ? "text-amber-600" : "text-stone-400 group-hover:text-stone-600"}`}
          />
        </motion.div>
      </button>
